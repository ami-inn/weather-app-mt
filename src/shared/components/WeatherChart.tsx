...
const getMetricConfig = (type: MetricType) => {
  const configs = {
    temperature: {
      color: '#FF6B6B',
      gradient: ['#FF6B6B', '#FFD6D6'],
      unit: '°C',
      title: 'Temperature',
    },
    rainfall: {
      color: '#4ECDC4',
      gradient: ['#4ECDC4', '#D0F5F2'],
      unit: 'mm',
      title: 'Rainfall',
    },
    soilTemperature: {
      color: '#45B7D1',
      gradient: ['#45B7D1', '#B8EAFB'],
      unit: '°C',
      title: 'Soil Temperature',
    },
    soilMoisture: {
      color: '#96CEB4',
      gradient: ['#96CEB4', '#E6F9F2'],
      unit: '%',
      title: 'Soil Moisture',
    }
  };
...
const WeatherChart: React.FC<WeatherChartProps> = ({ data, metricType, isLoading }) => {
  ...
  // SVG gradient for recharts
  const gradientId = `chart-gradient-${metricType}`;

  try {
    return (
      <div className="w-full rounded-lg  bg-gradient-to-br from-white via-gray-50 to-gray-50 p-4">
        <div className="flex justify-between items-center mb-2">
          ...
          <button
            ...
            className="text-xs text-gray-600 hover:text-gray-800 border border-gray-200 rounded px-2 py-1 transition duration-150 hover:bg-gray-100"
          >
            ...
            {metricType === 'rainfall' ? (
              ...
              >
                <defs>
                  <linearGradient id={gradientId} x1="0" y1="0" x2="0" y2="1">
                    <stop offset="0%" stopColor={config.gradient[0]} stopOpacity={0.7}/>
                    <stop offset="100%" stopColor={config.gradient[1]} stopOpacity={0.2}/>
                  </linearGradient>
                </defs>
                ...
                <Tooltip
                  ...
                  labelStyle={{ color: config.color, fontWeight: 600 }}
                />
                <Area
                  type="monotone"
                  dataKey="value"
              ...
              >
                ...
                <Line
                  ...
                  strokeWidth={4}
                  dot={{ fill: config.color, strokeWidth: 2, r: 5, stroke: '#fff' }}
